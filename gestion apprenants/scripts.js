...
function detailApprenant(numb){
   var rang=parseInt(numb);
    var enfants=ligneTab[rang].childNodes;
    
    for (var i=0; i<enfants.length;i++){
        if (enfants[i].innerHTML!=undefined){
            valeurs.push(enfants[i].innerHTML);
            console.log(enfants[i].innerHTML);
        }
